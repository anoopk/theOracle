...
function init(entity){
	try{
		var special = require(utils.pathToEntity(entity))	
		fs.readdir(config.dataLocation + "entities/" + entity + "/analysis", function(err, files){
			if(err){
				console.log("Entity not initialsed")
			}
			else{
				...
				streams.forEach(stream => {
					console.log(aggregate(special[stream]))
				})
			}
		})
	}
	catch(err){
		console.log("Correct your configuation, and try again")
	}
}
